...
public class Dictionary {
	 ...
	 * @return
	 */
	public static Dictionary addDic2MainDic(List<InputStream> inputStreamList)
	{
		if(singleton == null)
		{
	...
	private void loadWords2DictSegment(InputStream is,DictSegment dstDicSegment) {
		...
		if(is != null)
		{
			try {
				...
				do {
					theWord = br.readLine();
					if (theWord != null && !theWord.trim().isEmpty() && theWord.trim().charAt(0) !='#') {

						dstDicSegment.fillSegment(theWord.trim().toLowerCase()
								.toCharArray());
					}
				} while (theWord != null);
